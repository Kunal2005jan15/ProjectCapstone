...
public class InventoryController {
    ...
    /** Reorder suggestions driven by Prophet demand forecasts already stored in the `forecasts` table. */
    @GetMapping("/reorder-suggestions")
    public ResponseEntity<List<ReorderSuggestionResponse>> reorderSuggestions(
            @PathVariable UUID shopId, @RequestParam(defaultValue = "7") int days) {
        return ResponseEntity.ok(inventoryService.getReorderSuggestions(shopId, days));
    }
