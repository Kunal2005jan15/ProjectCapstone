...
# Override via env var in real deployment, e.g. your Render backend's URL:
#   BACKEND_BASE_URL=https://dine-shop-ai-backend.onrender.com
...
# Used only by the live pipeline (push_live_forecasts.py) — sent as the
# X-ML-Service-Key header, kept deliberately separate from the
# Authorization: Bearer JWT flow used for real logged-in users.
ML_PUSH_API_KEY_ENV_VAR = "ML_PUSH_AUTH_TOKEN"